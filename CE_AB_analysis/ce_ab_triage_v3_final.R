...
grid_decision <- expand.grid(
  ab_sens = seq(0.05, 0.99, length.out = 100),
...
calculate_strategy_thresholds <- function(params_list = params, wtp = 500) {
  ...
  sweep_grid <- expand.grid(
    ab_sens = seq(0.05, 0.999, by = 0.001),
  ...
  full_results %>%
    ...
    summarise(
      min_sens_health_gain = safe_first(ab_sens[delta_DALY > 0]),
      min_sens_cost_effective = safe_first(ab_sens[nmb > 0 & delta_DALY > 0]),
      min_sens_dominant = safe_first(ab_sens[delta_DALY > 0 & delta_cost < 0]),
      .groups = "drop"
    ) %>%
    # Formatted once with mutate across for clean, readable code
    mutate(
      across(
        c(min_sens_health_gain, min_sens_cost_effective, min_sens_dominant),
        ~ ifelse(is.na(.x), "Not Viable", scales::percent(.x, accuracy = 0.1))
      )
    )
}
...
fig_headline_threshold <- ggplot(sens_results, aes(x = ab_sens, y = delta_DALY, color = freq_label)) +
