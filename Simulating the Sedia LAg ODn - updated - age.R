...
poly_model <- nlme::lme(sedia_ODn ~ Age + poly(years_since_tx_start, 2), 
                  data = full_dataset %>%
                    filter(Group == 'early suppressions'), 
...
generate_decay_rate <- function(t) {
  ...
  a <- rnorm(1, mean = summary(poly_model)$tTable[,1][[4]], 
             sd = summary(poly_model)$tTable[,2][[4]] * sqrt(length(unique(sedia_eddi_data$subject_label_blinded))))#1.818730
  b <- rnorm(1, mean = summary(poly_model)$tTable[,1][[3]], 
             sd = summary(poly_model)$tTable[,2][[3]] * sqrt(length(unique(sedia_eddi_data$subject_label_blinded))))#-8.97018
  c <- rnorm(1, mean = summary(poly_model)$tTable[,1][[2]], 
             sd = summary(poly_model)$tTable[,2][[2]] * sqrt(length(unique(sedia_eddi_data$subject_label_blinded))))#3.254208
  d <- rnorm(1, mean = summary(poly_model)$tTable[,1][[1]], 
             sd = summary(poly_model)$tTable[,2][[1]] * sqrt(length(unique(sedia_eddi_data$subject_label_blinded))))#3.254208
  decay_rate <- a * t^2 + b * t + c + d
  return(c(pmax(decay_rate, 0.05), a, b, c, d))  # Ensure decay rates are non-negative
}
...
model_parameters <- bind_cols(id = 1:n_individuals,
  ...
  remove_rownames(data.frame(t(data.frame(decay_rates[c(22,23,24, 25),])))) %>%
  dplyr::select(a = X1, b = X2, c = X3, d = X4))
...
dt02 <- read_csv("data/20180410-EP-LAgSedia-Generic.csv") %>%
  ...
  dplyr::select(subject_label_blinded, visit_date, test_date, art_initiation_date, 
                days_since_eddi, viral_load, age_in_years, sedia_ODn = result...72) %>% 
  ...
  dplyr::select(subject_label_blinded, strata, years_since_ART_start, age_in_years, sedia_ODn, peak)
...
dt04 <- full_dataset %>%
  ...
  dplyr::select(subject_label_blinded, strata = Group, years_since_ART_start, age_in_years = Age, sedia_ODn, peak)
test_data <- bind_rows(
  dt02 %>%
    mutate(subject_label_blinded = as.character(subject_label_blinded)), dt04
) %>%
  mutate(id = as.numeric(as.factor(subject_label_blinded))) %>%
  dplyr::select(record_id = id, time_vec = years_since_ART_start, age_in_years,
...
best_model_choice <- function(test_data, param_sim_data) {
  ...
  for (i in 1:length(ODn_vec[[1]])) {
    t <- time_vec[[i,1]]
    y <- ODn_vec[[i,1]]
    dt[[paste0("square_error", i)]] <- (y - pmax(baselines * exp(-(a * t^2 + b * t + c + d) * t), .001))^2
  }
  # browser()
  ...
  dt2 <- dt1 %>%
    ...
    dplyr::select(id, baselines, a, b, c, d, rmse)
  return(dt2)
}
...
compare_value_with_others <- function(data_set, t, y_ODn, sigma_ODn, sigma_y_ODn, id) {
  ...
  y <- y_ODn
  y_hat <- (data_set %>%
              mutate(y_hat = pmax(baselines * exp(-(a * t^2 + b * t + c + d) * t), .001)))$y_hat
