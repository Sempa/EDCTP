...
pt_dataset <- read_excel("data/rv329_sacema_12sep22_Plasma Inventory[3].xlsx", sheet = "Request")
...
x_data <- pt_dataset %>%
  filter(`SUBJECT ID (CHAR)` %in% unique((final_dataset%>% mutate(country = grepl('A', `SUBJECT ID (CHAR)`)) %>% filter(ever_unsuppressed_after_artstart==1))$`SUBJECT ID (CHAR)`))
