...
slopes_for_suppressed <- function(ODn_vl_data, threshold) {
  ...
  model_data <- model_data %>%
    mutate(
      interceptlink_identity = as.numeric(interceptlink_identity),
...
for (i in c(1000)) { # 100, 400,
  x <- slopes_for_suppressed(
    ODn_vl_data = full_dataset %>%
...
model_data <- read_rds("data/model_data_art.rds") %>%
  mutate(
    sigma_slope_identity = (sd(slope_link_identity) / (length(slope_link_identity) - 1))^0.5 # ,
    # sigma_slope_log = (sd(slope_link_log)/(length(slope_link_log) - 1))^0.5
  ) %>%
  group_by(id) %>%
  mutate(
    z_value_identity = (slope_link_identity - 0) / sigma_slope_identity,
...
data_intermitent_suppression_selected_visits <- read_delim("data/full_africos_data_with_ODn.csv",
  ...
  select(
    subject_label_blinded = subjid, # days_since_eddi,
    Sex, Age,
  ...
  mutate(
    time_on_ART = as.numeric(as.Date(test_date, origin = "1900-01-01") - art_start),
    cohort = "africos",
    # subject_label_blinded = as.double(cur_group_id()),
    Age = as.numeric(Age),
    viral_load = as.numeric(viral_load)
...
slopes_for_unsuppressed <- function(ODn_vl_data, threshold) {
  ...
  print(
    ggplot(
      ...
      geom_line(aes(color = as.factor(subject_label_blinded)), size = 1) + # aes(color = id, linetype = Group), group = Group
      theme(
        text = element_text(size = 20),
        plot.title = element_text(hjust = 0.5),
        axis.line = element_line(colour = "black"),
...
# browser()
  ...
  # par(mfrow = c(1, 2))
  # hist(model_data$slope_link_identity, breaks = 30, freq = FALSE, xlim = c(-0.02, 0.04), ylab = "", xlab = "Slopes of individuals", main = "A", col = "black", cex.lab = 2, cex.axis = 2, xaxt = "n", cex.main = 2) # , main = paste("Density plot link=identity_", threshold)
  ...
  slope_figure_1000 <- ggplot(
    ...
    geom_histogram(color = "black", fill = "red", bins = 30, alpha = 0.9) +
    scale_x_continuous(limits = c(-0.006, 0.006), breaks = seq(-0.006, 0.007, by = 0.003), expand = c(0, 0)) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
    theme(
      text = element_text(size = 20),
      plot.title = element_text(hjust = 0.5),
...
results_suppressed1 <- as_tibble(results_suppressed) %>%
  ...
  mutate(
    significance = ifelse(as.numeric(p_value) < 0.05, TRUE, FALSE),
...
compare_lastvalue_with_previous <- function(data_set, sigma_ODn) {
  ...
  data_generated <- data_set %>% # data_intermitent_suppression_selected_visits
    filter(to_peak == 1) %>%
    mutate(id = as.character(subject_label_blinded)) %>%
    dplyr::select(id, time_on_ART, test_date, sedia_ODn, viral_load, to_peak, to_peak)
  results1 <- c()
  ids <- unique(data_generated$id)
  for (i in 1:length(ids)) {
    ODn_values1 <- (data_generated %>%
...
compare_first_peak_value <- as_tibble(
  ...
  mutate(
    `significance 99%` = ifelse(as.numeric(z_stat) > qnorm(0.99), TRUE, FALSE),
...
sensitivity_value <- cbind(`Accuracy values` = c(
  ...
  round(0 / (table(compare_first_peak_value$`significance 97.5%`)[[1]]), 3),
  round(0 / (table(compare_first_peak_value$`significance 98%`)[[1]]), 3),
  round(0 / (table(compare_first_peak_value$`significance 98.5%`)[[1]] ), 3),
  round(0/ (table(compare_first_peak_value$`significance 99%`)[[1]]), 3)
...
jpeg('other_figures/compare_lastvalue_with_previous_art.jpeg', units = "in", width = 8, height = 6, res = 300)
graph_compare_first_peak_value
dev.off()
...
# for (i in c(100, 400, 1000)) {
  # results1 <- c()
  results_suppressed <- rbind(results_suppressed, compare_value_with_previous(data_set = full_dataset %>%
                                                                                filter(Group == "early suppressions") %>%
                                                                                filter(time_on_ART > 0 & viral_load < 1000), threshold = 1000,
                                                                              sigma_ODn = sigma_ODn_func(full_dataset %>%
                                                                                                           filter(Group == "early suppressions") %>%
...
jpeg('other_figures/compare_value_with_previous_art.jpeg', units = "in", width = 8, height = 6, res = 300)
graph_results_suppressed2
dev.off()

jpeg('other_figures/report_figures_3_art.jpeg', units = "in", width = 15, height = 13, res = 300)
...
# accuracy_dataset
jpeg('other_figures/accuracy_barplot_art.jpeg', units = "in", width = 8, height = 6, res = 300)
accuracy_dataset
dev.off()
...
# accuracy_graph
# dev.off()
jpeg('other_figures/report_figure4_art.jpeg', units = "in", width = 8, height = 6, res = 300)
accuracy_graph
dev.off()